...
/**
 * The wiki_User object encapsulates all of the user-specific settings (user_id,
...
class wiki_User {
	 ...
	 * Use the wiki_User::newFrom* factory functions for other kinds of users.
	 *
	 * @see newFromName()
	 * @see newFromId()
	...
	public function loadFromId() {
		...
		if ( !$data ) {
			wfDebug( "wiki_User: cache miss for user {$this->mId}\n" );
			# Load from DB
			...
			wfDebug( "wiki_User: got user {$this->mId} from cache\n" );
			# Restore from cache
	 ...
	 *    for 'valid', for BC.
	 *
	 * @return wiki_User object, or false if the username is invalid
	...
	public function getPasswordValidity( $password ) {
		...
		static $blockedLogins = array(
			'wiki_Useruser' => 'Passpass', 'wiki_Useruser1' => 'Passpass1', # r75589
	...
	private function loadFromSession() {
		...
		if( $wgBlockDisablesLogin && $proposedwiki_User->isBlocked() ) {
			# wiki_User blocked and we've disabled blocked user logins
			$this->loadDefaults();
			return false;
		}
	 ...
	 */
	protected function loadFromwiki_UserObject( $user ) {
		$user->load();
		$user->loadGroups();
		$user->loadOptions();
	...
	private function getBlockedStatus( $bFromSlave = true ) {
		...
		if ( $block instanceof Block ) {
			...
			$this->mAllowwiki_Usertalk = !$block->prevents( 'editownusertalk' );
		} else {
			$this->mBlockedby = '';
			$this->mHideName = 0;
	...
	public function isLocked() {
		...
		global $wgAuth;
		$authwiki_User = $wgAuth->getUserInstance( $this );
	 ...
	 * remote IP.
	 *
	 * @note wiki_User::newFromName() has rougly the same function, when the named user
	 * does not exist.
	 ...
	 * @param $id String|Int wiki_User's IP address for anonymous users, wiki_User ID otherwise
	 ...
	 *
	 * @return wiki_User object, or null if the username already exists
	 */
	public static function createNew( $name, $params = array() ) {
		...
		if ( $dbw->affectedRows() ) {
			$newwiki_User = wiki_User::newFromId( $dbw->insertId() );
		} else {
			$newwiki_User = null;
		}
		return $newwiki_User;
	...
	* Return a URL the user can use to confirm their email address.
	 ...
	 *
	 * @param $group String Internal group name
	 * @param $username String wiki_Username for gender (since 1.19)
	 ...
	 * @param $userId String wiki_User ID
	 ...
	 * @param $userId String|bool wiki_User ID for old-style password salt
	 *
	 * @return Boolean
	 */
	public static function comparePasswords( $hash, $password, $userId = false ) {
		...
		$result = false;
		if( !wfRunHooks( 'wiki_UserComparePasswords', array( &$hash, &$password, &$userId, &$result ) ) ) {
			return $result;
		}
	...
	protected function loadOptions() {
		...
		if ( !is_null( $this->mOptionOverrides ) ) {
			wfDebug( "wiki_User: loading options for user " . $this->getId() . " from override cache.\n" );
			foreach( $this->mOptionOverrides as $key => $value ) {
				$this->mOptions[$key] = $value;
			}
		} else {
			wfDebug( "wiki_User: loading options for user " . $this->getId() . " from database.\n" );
			// Load from database
			$dbr = wfGetDB( DB_SLAVE );
	...
	protected function saveOptions() {
		...
		$extuser = Externalwiki_User::newFromwiki_User( $this );
		$userId = $this->getId();
		$insert_rows = array();
