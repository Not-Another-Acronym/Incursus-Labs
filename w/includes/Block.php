...
class Block {
		...
		$hideName = 0, $blockEmail = 0, $allowwiki_Usertalk = 0, $byText = '' )
	{
		...
		if ( $this->target instanceof wiki_User && $user ) {
		...
		$this->prevents( 'editownusertalk', !$allowwiki_Usertalk );
	...
	public static function newFromDB( $address, $user = 0 ) {
		...
		return self::newFromTarget( wiki_User::whoIs( $user ), $address );
	}
	 ...
	 *     * A user to block, in which case $target will be a wiki_User
	...
	public static function parseTarget( $target ) {
		...
		$userObj = wiki_User::newFromName( $target );
		if ( $userObj instanceof wiki_User ) {
	 ...
	 * @return wiki_User|string Local wiki_User object or string for a foreign user
	 */
	 ...
	 * @param $user wiki_User|string Local wiki_User object or username string for foriegn users
	 */
