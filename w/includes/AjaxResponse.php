	...
	function checkLastModified ( $timestamp ) {
		...
		if ( !empty( $_SERVER['HTTP_IF_MODIFIED_SINCE'] ) ) {
			...
			if ( ( $ismodsince >= $timestamp ) && $wgwiki_User->validateCache( $ismodsince ) && $ismodsince >= $wgCacheEpoch ) {
				...
				wfDebug( "$fname: CACHED client: $ismodsince ; user: {$wgwiki_User->getTouched()} ; page: $timestamp ; site $wgCacheEpoch\n", false );

				return true;
			} else {
				wfDebug( "$fname: READY  client: $ismodsince ; user: {$wgwiki_User->getTouched()} ; page: $timestamp ; site $wgCacheEpoch\n", false );
				$this->mLastModified = $lastmod;
			}
		} else {
