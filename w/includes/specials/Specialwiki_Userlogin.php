...
class LoginForm extends SpecialPage {
	...
	function addNewAccountInternal() {
		...
		if( 'local' != $this->mDomain && $this->mDomain != '' ) {
			if( !$wgAuth->canCreateAccounts() && ( !$wgAuth->userExists( $this->mwiki_Username )
		...
		if ( !$currentwiki_User->isAllowed( 'createaccount' ) ) {
			...
			$this->userBlockedMessage( $currentwiki_User->isBlockedFromCreateAccount() );
			return false;
		}

		# Include checks that will include GlobalBlocking (Bug 38333)
		$permErrors = $this->getTitle()->getUserPermissionsErrors( 'createaccount', $currentwiki_User, true );
		if ( count( $permErrors ) ) {
	...
	public function authenticateUserData() {
		...
		// potentially creates the user in the database. Until we load $wgwiki_User,
		// checking for user existence using wiki_User::newFromName($name)->getId() below
		// will effectively be using stale data.
		if ( $this->getUser()->getName() === $this->mwiki_Username ) {
		...
		if ( !$u->checkPassword( $this->mPassword ) ) {
			...
			if ( $isAutoCreated ) {
				// Must be run after $wgwiki_User is set, for correct new user log
