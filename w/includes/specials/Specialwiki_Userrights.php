...
class wiki_UserrightsPage extends SpecialPage {
	...
	public function execute( $par ) {
		...
		if ( wiki_User::getCanonicalName( $this->mTarget ) == $user->getName() ) {
			$this->isself = true;
		}
	 ...
	 * @return null
	 */
	function savewiki_UserGroups( $username, $reason = '' ) {
		$status = $this->fetchwiki_User( $username );
		if( !$status->isOK() ) {
		...
		$this->doSavewiki_UserGroups( $user, $addgroup, $removegroup, $reason );
	}
	 ...
	 */
	function doSavewiki_UserGroups( $user, $add, $remove, $reason = '' ) {
		// Validate input set...
		...
		wfRunHooks( 'wiki_UserRights', array( &$user, $add, $remove ) );
	 ...
	 */
	function editwiki_UserGroupsForm( $username ) {
		$status = $this->fetchwiki_User( $username );
		if( !$status->isOK() ) {
			$this->getOutput()->addWikiText( $status->getWikiText() );
			return;
		...
		$this->showEditwiki_UserGroupsForm( $user, $groups );
	 ...
	 */
	public function fetchwiki_User( $username ) {
		global $wgwiki_UserrightsInterwikiDelimiter;

		$parts = explode( $wgwiki_UserrightsInterwikiDelimiter, $username );
		if( count( $parts ) < 2 ) {
			$name = trim( $username );
			$database = '';
		} else {
		...
		if( $database == '' ) {
			...
			$user = wiki_UserRightsProxy::newFromName( $database, $name );
		}
	 ...
	 *
	 * @param $user      wiki_User or wiki_UserRightsProxy you're editing
	 * @param $groups    Array:  Array of groups the user is in
	 */
	protected function showEditwiki_UserGroupsForm( $user, $groups ) {
		$list = array();
	 ...
	 * @param $user wiki_User a user object
	...
	private function groupCheckboxes( $usergroups, $user ) {
		...
		foreach( $columns as $column ) {
			...
			foreach( $column as $group => $checkbox ) {
				...
				$member = wiki_User::getGroupMember( $group, $user->getName() );
	 ...
	 *
	 * @param $user wiki_User to show log for
