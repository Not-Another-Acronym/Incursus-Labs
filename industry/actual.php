    	...
    	<div id="wrap">
	        <?php
		        ...
		        while($r=$q->fetch_array()){
		                if(!in_array($r["typeID"],$building))
		                {
		                        ...
		                        $resTable .= "<tr><td>".$r["journalTransactionID"]."</td><td>".$r["typeName"]."</td><td>" .
		                                number_format($profit,2) . "</td><td>" . number_format($profit/$r["quantity"],2) . "</tr>";
		        ...
		        foreach($monthProfit as $i=>$v){
		                $sparrow=(($v-400000000)*.05);
		                if($i=="11/2012")
		                        $sparrow = 0;
		                $dm = split("/",$i);
		                if($dm[1] > 2013 || ($dm[1] == 2013 && $dm[0] >= 3))
		                        $sparrow -= 400000000;
		                print($i . " = " . number_format($v,2) . " To Sparrow " . number_format($sparrow,2) . " True Profit " . number_format($v-$sparrow,2) . "<br>");
		                $total += $v-$sparrow;
		        }
		        print("Total:" . number_format($total));
		        print("<a href='image.php'><img src='image.php' width=100%></a>" . $resTable . "</table>");
