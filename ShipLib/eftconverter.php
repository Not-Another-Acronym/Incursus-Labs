...
foreach ($fittingarray as $line)
{
	...
	$count = array(1,1);
	$matches=array();
	if(preg_match("/^(.*)x([0-9]+)$/",$moduledetail[0],$matches))
	{
	    $count[0] = $matches[2];
	    $moduledetail[0] = trim($matches[1]);
	}
	if(array_key_exists(1,$moduledetail) && preg_match("/^(.*)x([0-9]+)$/",$moduledetail[1],$matches))
        {
            $count[1] = $matches[2];
            $moduledetail[1] = trim($matches[1]);
        }
        if(array_key_exists($moduledetail[0],$mods))
        {
            $mods[$moduledetail[0]]+=$count[0];
        }
        else
        {
            $mods[$moduledetail[0]]=$count[0];
